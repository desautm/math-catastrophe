...
subtitle: "... quand l'informatique s'emmêle les bits"
author: "Marc-André Désautels"
format: 
  revealjs:
    incremental: true
    logo: logo-cegep.jpg
    footer: "desautm.github.io/math-catastrophe"
editor: visual
...
# Les erreurs de débordement {background-color="black"}
...
## {background-image="https://upload.wikimedia.org/wikipedia/commons/5/53/Odometer_rollover.jpg" background-color="black" background-size="contain"}

# La représentation des nombres {background-color="black"}
...
$$
\begin{aligned}
(a_na_{n-1}\cdots a_1a_0,a_{-1}a_{-2}\cdots a_{-m})_b &= \sum_{k=-m}^n a_k b^k
\end{aligned}
$$ 
...
# Les entiers non signés {background-color="black"}
...
<!-- - Pire encore, si une opération arithmétique produit une valeur supérieure à cette limite, il se produit un **dépassement d’entier**. -->
...
# NUCLEAR GANDHI {background-color="black"}

## {background-image="https://upload.wikimedia.org/wikipedia/commons/thumb/7/74/Civ1_logo_v1.svg/1024px-Civ1_logo_v1.svg.png" background-size="contain"}
...
## {background-image="https://upload.wikimedia.org/wikipedia/en/a/a7/CivilizationAmigaAGA.png" background-color="black" background-size="contain"}

## {background-image="https://upload.wikimedia.org/wikipedia/commons/3/3b/Nuclear_Gandhi.png" background-size="contain" background-color="black"}

## {background-image="https://www.kxnet.com/wp-content/uploads/sites/16/2023/02/5f9.jpg" background-size="contain"}

## NUCLEAR GANDHI

- Dans le jeu original, lorsque le personnage de Gandhi est contrôlé par l'ordinateur, ce personnage pouvait utiliser l'arme atomique lors des conflits.
...
## {background-image="https://www.kxnet.com/wp-content/uploads/sites/16/2023/02/gandhi.png?resize=772,488" background-size="contain"}
...
## {background-color="black" background-image="pacman_256.gif" background-size="contain"}
...
- Le logiciel utilisait un fanion sur 8 bits et l'incrémentait, plutôt que d'utiliser un booléen (`True` ou `False`)
...
# Donkey Kong {background-color="black"}

# {background-image="Donkey_Kong_Gameplay.png" background-color="black" background-size="contain"}

## Donkey Kong

- Dans le jeu d'arcade Donkey Kong, il est impossible d'avancer plus loin que le niveau 22.
- Le jeu calcul le temps/bonus en prenant le numéro du niveau, en le multipliant par 10 et en additionnant 40.
- Lorsqu'on atteint le niveau 22, le temps/bonus est égal à 260, ce qui est plus grand que la valeur maximale de 255 (les niveaux sont stockés dans un entier non signé sur 8 bits).

## Donkey Kong

- Le temps/bonus devient égal à 4, ce qui est trop petit pour terminer le niveau.
...
niveau = np.array(22).astype('uint8')
temps_bonus = np.array(niveau*10+40).astype('uint8')
print(niveau, temps_bonus)
```

# Les entiers signés {background-color="black"}
...
- Contrairement aux entiers non signés, le débordement fonctionne différemment avec les entiers signés, en raison du complément à deux.
...
print("Valeur réelle", "Valeur signée".rjust(20))
...
# Super Mario Bros. {background-color="black"}

# {background-image="https://upload.wikimedia.org/wikipedia/en/0/03/Super_Mario_Bros._box.png" background-color="black" background-size="contain"}

# {background-image="https://upload.wikimedia.org/wikipedia/en/5/50/NES_Super_Mario_Bros.png" background-color="black" background-size="contain"}

#

<iframe width="1050" height="700" src="https://www.youtube.com/embed/0X9_K9prpVc?si=Mfzw69uU0n0KM73p" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
::: {.notes}
[Speaker notes go here.](https://themushroomkingdom.net/smb_breakdown.shtml)

Niveau -1 dans SMB
:::

# {background-image="mario.png" background-color="black" background-size="contain"}

# {background-image="mario-lives.png" background-color="black" background-size="contain"}

# Gangnam Style {background-color="black"}

##

<iframe width="1050" height="700" src="https://www.youtube.com/embed/9bZkp7q19f0?si=0XcKi7VkLiAKGYRi" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

## Gangnam Style

- Bien qu'on ne soit plus en 2012, les gens regardent encore le vidéoclip GangNam Style de Psy sur Youtube.
- Au moment où j'écris ces lignes, le nombre de visionnements est de ...
...
- Lorsque YouTube a été développé, personne n'imaginait qu'une vidéo pourrait être regardée plus de 2 milliard de fois!
...
max = np.array(2**31-1).astype('int32')
un  = np.array(1).astype('int32')
print(max+un)
```
...
- Le bug apparaît si la somme de `low` et `high` est plus grande que la valeur maximum possible pour un entier signé dans `Java`, $2^{31}-1$. 
...
if low > high:
  low, high = high, low
print(low, high, round((low + high) / 2))
```
...
if low > high:
  low, high = high, low
print(low, high, round(low + (high - low) / 2))
```
...
# GPS week number rollover {background-color="black"}

## GPS week number rollover

- La date diffusée par le GPS contient le nombre de semaines depuis le 6 janvier 1980.
- Elle est stockée sur dix bits, donc une plage de 0 à 1 023.
- Il y a donc un débordement à chaque 1 024 semaines, ou 19,6 ans.
- Les logiciels qui ne sont pas codés pour anticiper le retour à zéro peuvent cesser de fonctionner ou reculer de 20 ou 40 ans.
...
```{.fortran code-line-numbers="11-13"}
L_M_BV_32 := TBD.T_ENTIER_16S((1.0 / C_M_LSB_BH) *
                                   G_M_INFO_DERIVE(T_ALG.E_BH));
if L_M_BV_32 > 32767 then
 P_M_DERIVE(T_ALG.E_BV) := 16#7FFF#;
